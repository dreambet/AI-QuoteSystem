  ...
  return <div className="quote-detail-page">
    ...
    <div className="detail-layout">
      <main className="detail-main-column">
        <section className="detail-card overview-card"><div className="detail-section-title"><div><span className="eyebrow">PART OVERVIEW</span><h2>零件与图纸信息</h2></div><span className="drawing-type-chip">{drawingType(quote.drawingPath)}</span></div><div className="overview-grid"><div><span>物料编码</span><strong>{quote.materialCode || '-'}</strong></div><div><span>材料</span><strong>{quote.material || '—'}</strong></div><div><span>数量</span><strong>{quote.quantity || 1} 件</strong></div><div><span>毛重 / 净重</span><strong>{quote.grossWeight ?? '-'} / {quote.netWeight ?? '-'} kg</strong></div><div><span>毛坯尺寸</span><strong>{quote.blankSpec?.['料长'] || '—'} × {quote.blankSpec?.['料宽'] || '—'} × {quote.blankSpec?.['料厚'] || '—'} mm</strong></div><div><span>外径</span><strong>{quote.blankSpec?.['外径'] ? `${quote.blankSpec['外径']} mm` : '—'}</strong></div><div className="overview-file"><span>关联图纸</span><strong>{drawingName(quote.drawingPath)}</strong></div></div></section>

        <section className="detail-card analysis-detail-card"><div className="detail-section-title"><div><span className="eyebrow">AI DRAWING ANALYSIS</span><h2>图纸识别结果</h2></div><b>{features.length} 项特征</b></div><div className="analysis-detail-grid"><div className="feature-result-panel"><div className="feature-result-heading"><span>识别特征</span><small>滚动查看全部</small></div>{features.length ? <div className="detail-feature-scroll">{features.map((feature, index) => <div className="detail-feature-row" key={`${feature.type}-${index}`}><span>#{String(index + 1).padStart(2, '0')}</span><div><strong>{feature.type || 'CAD 特征'}</strong><small>{feature.description || '已从图纸几何信息中识别'}</small></div></div>)}</div> : <div className="detail-empty-inline">该任务尚未生成特征识别结果。</div>}</div><div className="analysis-meta-panel"><div><span>识别实体</span><strong>{analysis.cadInfo?.entityCount ?? '—'}</strong></div>{analysis.tolerances && <div><span>一般公差</span><strong>{analysis.tolerances.general || '—'}</strong></div>}<div className="analysis-note"><span>分析备注</span><p>{analysis.notes || '暂无额外图纸分析备注。'}</p></div></div></div></section>
